...
alt.onServer("Client:HUD:CreateCEF", (hunger, thirst, armor, health, currentmoney, currentmoneyblack) => {
    if (hudBrowser == null) {
        hudBrowser = new alt.WebView("http://resource/client/cef/hud/index.html");
        hudBrowser.on("Client:HUD:cefIsReady", () => {
            alt.setTimeout(function() {
                hudBrowser.emit("CEF:HUD:updateDesireHUD", hunger, thirst, armor, health);
        ...
        hudBrowser.on("Client:Animation:SaveAnimationHotkey", (hotkey, animId, animName, animDict, animFlag, animDuration) => {
            ...
            if (hotkey != "Num1" && hotkey != "Num2" && hotkey != "Num3" && hotkey != "Num4" && hotkey != "Num5" && hotkey != "Num6" && hotkey != "Num7" && hotkey != "Num8" && hotkey != "Num9") {
                alt.emit("Client:HUD:sendNotification", 4, 3500, "The content of the hotkey may only consist of Num1 to Num9.");
                return;
            }
            ...
            alt.emit("Client:HUD:sendNotification", 2, 2500, `Hotkey ${hotkey} successfully occupied - animation ID: ${animId}.`);
        });
        ...
        hudBrowser.on("Client:Animation:DeleteAnimationHotkey", (hotkey) => {
            ...
            alt.emit("Client:HUD:sendNotification", 2, 2500, `Hotkey ${hotkey} successfully removed.`);
        });
...
alt.onServer("Client:HUD:setIntoVehicle", (vehicle) => {
    alt.setTimeout(() => {
        game.setPlayerCanDoDriveBy(alt.Player.local.scriptID,false);
...
alt.onServer("Client:HUD:UpdateDesire", (hunger, thirst, armor, health) => {
    if (hudBrowser != null) {
        hudBrowser.emit("CEF:HUD:updateDesireHUD", hunger, thirst, armor, health);
    }
});
...
alt.on('keydown', (key) => {
    ...
    // }
        ...
        if (currentRadioFrequence == null || currentRadioFrequence == undefined  || alt.Player.local.getSyncedMeta("IsCefOpen")) return;
        alt.emit("SaltyChat:UseRadio", true, true);
    }  else if (key == 'Z'.charCodeAt(0) && !alt.Player.local.getSyncedMeta("IsCefOpen")) { // Z (Y auf englischer Tastatur)
        alt.emit("SaltyChat:ToggleRange");
    } else if (key == 'B'.charCodeAt(0) && alt.Player.local.vehicle && alt.Player.local.scriptID == game.getPedInVehicleSeat(alt.Player.local.vehicle.scriptID, -1) && game.getVehicleClass(alt.Player.local.vehicle.scriptID) == 18) {
        isPlayerUsingMegaphone = true;
        alt.emit("SaltyChat:UseMegaphone", true);
...
alt.on('keyup', (key) => {
    if (key == 'X'.charCodeAt(0)) {
        ...
        if (!isPhoneEquipped) {alt.emit("Client:HUD:sendNotification", 4, 5000, "You Don't Have a Phone"); return;}
        ...
        if (hudBrowser == null || !browserReady || isPlayerDead || alt.Player.local.getSyncedMeta("IsCefOpen") == true || alt.Player.local.getSyncedMeta("HasHandcuffs") == true || alt.Player.local.getSyncedMeta("HasRopeCuffs") == true)  return;
        alt.emitServer("Server:CEF:setCefStatus", true);
...
function InterActionMenuDoAction(type, action) {
    if (selectedRaycastId != null && selectedRaycastId != 0 && type != "none") {
        ...
        if (type == "vehicle") {
            ...
            } else if (action == "showLicensesCard") {
                alt.emitServer("Server:Raycast:showLicensesCard", playerRC);
...
function InterActionMenuDoActionAnimationMenu(action) {
    new Promise((resolve, reject) => {
        ...
        console.log("Animation Error occurred...")
    });
}
...
function InterActionMenuDoActionClothesRadialMenu(action) {
    new Promise((resolve, reject) => {
        ...
        console.log("Clothing Menu Error occurred...")
    });
}
...
alt.onServer("Client:WeaponAmmoChange:ComingRespond", (weaponhash) => {
    if (alt.Player.local.currentWeapon != null && alt.Player.local.currentWeapon == weaponhash) {
        ...
        alt.emit("Client:HUD:sendNotification", 4, 4500, "Done: Weapon down! (+" + ammo + " shot)");
    }
    else {
        alt.emit("Client:HUD:sendNotification", 4, 4500, "Mistake: You have to hold the gun in your hand");
    }
});
