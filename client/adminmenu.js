...
alt.onServer("Client:HUD:blackout", (status) => {
    if(status == "on"){
...
alt.onServer("Client:AdminMenu:GetWaypointInfo", () => {
    ...
    //     var [found, groundZ] = game.getGroundZFor3dCoord(coords.x, coords.y, coords.z + 100, undefined, undefined);
    //     coords = new alt.Vector3(coords.x, coords.y, groundZ + 1);
    //     // game.requestCollisionAtCoord(coords.x, coords.y, groundZ + 1);
    //     alt.emitServer('Server:AdminMenu:TeleportWaypoint', coords.x, coords.y, coords.z);
...
alt.onServer("Client:AdminMenu:Spectate", (target, info) => {
    if (info == "on") {
        ...
        game.setEntityCoords(alt.Player.local.scriptID, target.pos.x, target.pos.y, target.pos.z - 3, 1, 0, 0, 1);
        alt.setTimeout(() => {
        ...
        game.setEntityCoords(alt.Player.local.scriptID, spectate_lastpos.x, spectate_lastpos.y, spectate_lastpos.z, 1, 0, 0, 1);
    ...
    if (info == "on") {
        nametags_tick = alt.everyTick(() => {
            for (let i = 0, n = alt.Player.all.length; i < n; i++) {
                ...
                if (!game.isEntityDead(player.scriptID,0)) {
                    drawBarBackground(100, lineHeight, scale, 0.25, 0, 147, 29, 255, 0);
                    drawBar(game.getEntityHealth(player.scriptID) - 100, lineHeight, scale, 0.25, 0, 224, 11, 255, 0);
            
                    if (game.getPedArmour(player.scriptID) > 0) {
                        drawBarBackground(100, lineHeight, scale, 0.75, 0, 45, 142, 255, 0);
                        drawBar(game.getPedArmour(player.scriptID), lineHeight, scale, 0.75, 0, 74, 234, 255, 0);
                    }
                }
...
function drawBarBackground(value, lineHeight, scale, position, r, g, b, a, p8) {
    const width = value * 0.0005 * scale;
    game.drawRect(0, lineHeight + position * lineHeight, width + 0.002, lineHeight / 3 + 0.002, 0, 0, 0, 255, 0);
    game.drawRect(0, lineHeight + position * lineHeight, width, lineHeight / 3, r, g, b, a, p8);
}
...
export default class NoClip {
    ...
    static keyHandler() {
        ...
        if (!isVectorEqual(new alt.Vector3(currentPos.x, currentPos.y, currentPos.z + zModifier), alt.Player.local.pos)) game.setEntityCoords(alt.Player.local.scriptID, currentPos.x, currentPos.y, (currentPos.z + zModifier - 1), 1, 0, 0, 1);
    }
}